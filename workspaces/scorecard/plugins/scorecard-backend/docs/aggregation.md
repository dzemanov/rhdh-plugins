...
Requires user authentication. The endpoint uses the authenticated user's catalog entity reference to resolve owned entities (same ownership rules as the snapshot route).

#### Permissions

Requires **`scorecard.metric.read`** for the KPI's underlying metric. Additionally:

- The user must be allowed to read that metric (returns **`403 Forbidden`** if access is denied)
- The user must have **`catalog.entity.read`** for **each** owned entity included in the aggregation
...
**`statusGrouped`**, **`weightedStatusScore`**, or any other type that does not implement time-series:

- **Status code**: `400 Bad Request`
- **Error**: `InputError` stating that the aggregation type does not support time-series (currently only scalar types are supported)

##### Invalid query parameters

Missing **`from`** / **`to`**, non-ISO datetimes, **`from`** after **`to`**, or a span greater than **365 days**:

- **Status code**: `400 Bad Request`
- **Error**: `InputError` describing the validation failure

##### Missing user entity reference

If the authenticated user does not have an entity reference in the catalog:
...
If the user entity does not exist in the catalog:
...
If the user does not have permission to read a specific owned entity:

- **Status code**: `403 Forbidden`
- **Error**: Permission denied for that entity

##### Metric access denied

If the user does not have access to the underlying metric:

- **Status code**: `403 Forbidden`
- **Error**: `NotAllowedError: To view the aggregation of a scorecard metric, your administrator must grant you the required permission.`
...
This route resolves KPI metadata from app-config and the metric registry. It does **not** load owned entities or evaluate per-entity **`catalog.entity.read`**. Snapshot and time-series routes still require **`scorecard.metric.read`** and **`catalog.entity.read`** as described below.

#### Permissions and errors (snapshot and time-series)

[`GET /aggregations/:aggregationId`](#get-aggregationsaggregationid) and [`GET /aggregations/:aggregationId/time-series`](#get-aggregationsaggregationidtime-series) both require:

- **`scorecard.metric.read`** on the underlying metric
...
| Per-entity denial                | `403 Forbidden`    | User cannot read a specific owned entity.                         |
| Invalid time-series query params | `400 Bad Request`  | Time-series only (`from` / `to` validation).                      |
| Unsupported time-series type     | `400 Bad Request`  | Time-series only (`statusGrouped` / `weightedStatusScore` today). |
...
5. **Metric access**: Aggregation snapshot and time-series routes enforce **`scorecard.metric.read`** for the underlying metric and **`catalog.entity.read`** for each included entity; expect **`403 Forbidden`** when either check fails. Time-series also requires a **scalar** KPI, plus valid **`from`** / **`to`** query parameters (ISO-8601, **`from` ≤ `to`**, maximum **365 days**); unsupported types and invalid ranges fail with **`400 Bad Request`**.
